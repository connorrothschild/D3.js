async function drawChart() {
	...
	const xScale = d3.scaleLinear().domain([ 0, 1 ]).range([ 0, dimensions.boundedWidth ]).clamp(true);

	const startYScale = d3.scaleLinear().domain([ sesIds.length, -1 ]).range([ 0, dimensions.boundedHeight ]);

	const endYScale = d3.scaleLinear().domain([ educationIds.length, -1 ]).range([ 0, dimensions.boundedHeight ]);
	...
	const startingLabelsGroup = bounds.append('g').style('transform', 'translateX(-20px)');
	...
	const endingLabelsGroup = bounds.append('g').style('transform', `translateX(${dimensions.boundedWidth + 20}px)`);
	...
	const femaleLegend = legendGroup
		.append('g')
		.attr('transform', `translate(${-dimensions.endsBarWidth * 1.5 + dimensions.endingBarPadding + 1}, 0)`);
	...
	const endingBarGroup = bounds.append('g').attr('transform', `translate(${dimensions.boundedWidth}, 0)`);
	...
	function updateMarkers(elapsed) {
		...
		females.enter().append('circle').attr('class', 'marker marker-circle').attr('r', 5.5).style('opacity', 0);
		females.exit().remove();
		...
		const endingGroups = educationIds.map((endId) =>
			people.filter((d) => xProgressAccessor(d) >= 1 && educationAccessor(d) == endId)
		);
		const endingPercentages = d3.merge(
			endingGroups.map((peopleWithSameEnding, endingId) =>
				d3.merge(
					sexIds.map((sexId) =>
						sesIds.map((sesId) => {
							const peopleInBar = peopleWithSameEnding.filter((d) => sexAccessor(d) == sexId);
							const countInBar = peopleInBar.length;
							const peopleInBarWithSameStart = peopleInBar.filter((d) => sesAccessor(d) == sesId);
							const count = peopleInBarWithSameStart.length;
							const numberOfPeopleAbove = peopleInBar.filter((d) => sesAccessor(d) > sesId).length;
		...
		endingBarGroup
			...
			.attr('x', (d) => -dimensions.endsBarWidth * (d.sexId + 1) - d.sexId * dimensions.endingBarPadding)
			.attr('width', dimensions.endsBarWidth)
			.attr(
				'y',
				(d) => endYScale(d.endingId) - dimensions.pathHeight / 2 + dimensions.pathHeight * d.percentAbove
			)
