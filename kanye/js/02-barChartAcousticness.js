function barPlotAcousticness(data, response) {
	/**********************
    ***** BASIC SETUP *****
    **********************/
    ...
    ***************************************/
	...
	svg
		...
		.call(d3.axisBottom(xScaleAcousticness));

	svg.selectAll('.xLabel').data([ { label: 'Acousticness' } ]).transition().duration(DURATION).text((d) => d.label);
    ...
    ***************************************/
	...
	svg
		...
		.call(d3.axisLeft(yScaleAcousticness));
    ...
    **********************/
	...
	if (response.direction === 'down') {
		...
		// 	.attr('y', function(d) {
		// 		return yScaleAcousticness(d.track_name) + yScaleAcousticness.bandwidth() / 2 + 4;
		// 	})
		// 	.attr('x', 0)
		// 	.transition()
		// 	.duration(1000)
		...
		// 	.attr('x', function(d) {
		// 		return xScaleAcousticness(d.acousticness) + 5;
		// 	})
		// 	.text(function(d) {
		// 		return d.acousticness;
		// 	});
		...
		plot
			...
			.attr('class', 'bar')
			.attr('y', function(d) {
				return yScaleAcousticness(d.track_name);
			})
			.attr('height', yScaleAcousticness.bandwidth())
			.attr('x', 0)
			.attr('width', 0)
			.transition()
			.duration(1000)
			.attr('width', function(d) {
				return xScaleAcousticness(d.acousticness);
			})
			.attr('fill', 'steelblue');
